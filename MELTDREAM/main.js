...
// ---- 滑杆参数 ----
const params = { flow: 1.3, wake: 1, cycle: 40, speed: 1.2, relief: 150, gloss: 1.0, glow: 0.7, hue: 0.15, glitter: 0.6 };
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  playShimmer(0.9);   // 水钻落水的脆响
});
// ═══════════════════ 水钻贴纸结束 ═══════════════════

// ═══════════════════════════════════════════════════════════
// 声音：呼吸 + 阶段音频（Web Audio 程序合成，无任何素材）
//   平静期   ：7 秒周期的"呼吸"（带通噪声 + LFO 包络）
//   爆发瞬间 ：低频心跳轰鸣（双击）
//   高速亢奋 ：低频嗡鸣层随爆发包络增强 + 随机细碎闪烁音
//   恢复期   ：嗡鸣/闪烁随包络自然消退，呼吸回归
// 浏览器要求首次用户点击才能出声 → 用"♪ 声音"胶囊开关
// ═══════════════════════════════════════════════════════════
let audio = null, audioOn = true, booms = 0;   // 声音默认打开
let lastPh = -1;
...
function initAudio() {
  ...
  noise.connect(bp).connect(breathGain).connect(master);
  noise.start(); lfo.start();

  // 亢奋低鸣：双失谐锯齿 + 低通，增益跟随爆发包络
  ...
  const droneGain = ctx.createGain(); droneGain.gain.value = 0.035;   // 恒定背景低鸣
  o1.connect(lp); o2.connect(lp); lp.connect(droneGain).connect(master);
  o1.start(); o2.start();

  audio = { ctx, master, breathGain, droneGain, lfo, booms: 0 };
}
...
function playBoom(strength = 1) {           // 低频心跳轰鸣（两连击）
  if (!audioOn || !audio) return;
  const { ctx, master } = audio;
  const mk = (delay, amp) => {
    const t = ctx.currentTime + delay;
    const o = ctx.createOscillator(); o.type = 'sine';
    o.frequency.setValueAtTime(94, t);
    o.frequency.exponentialRampToValueAtTime(36, t + 0.5);
    const g = ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.exponentialRampToValueAtTime(Math.max(0.0002, amp), t + 0.02);
    g.gain.exponentialRampToValueAtTime(0.0001, t + 0.85);
    o.connect(g).connect(master);
    o.start(t); o.stop(t + 1);
  };
  mk(0, 0.42 * strength);
  mk(0.26, 0.22 * strength);
  audio.booms++; booms = audio.booms;
}

function playShimmer(strength = 1) {        // 细碎闪烁音（点击生成贴纸时触发）
  if (!audioOn || !audio) return;
  const { ctx, master } = audio;
  const t = ctx.currentTime;
  const o = ctx.createOscillator(); o.type = 'sine';
  o.frequency.value = 1500 + Math.random() * 2400;
  const g = ctx.createGain();
  g.gain.setValueAtTime(0.030 * strength, t);
...
soundBtn.addEventListener('click', (e) => {
  ensureAudio();
  audioOn = !audioOn;
  soundTxt.textContent = audioOn ? '声音：开' : '声音：关';
  soundBtn.classList.toggle('on', audioOn);
  e.currentTarget.blur();
});

// 声音默认打开,但受浏览器自动播放策略限制:首次点击画面/按键时自动激活音频
ensureAudio();
...
// [调试] 循环状态探针
window.__phase = () => {
  const p = ((time % params.cycle) + params.cycle) % params.cycle;
  const attack = smoothstepJS(params.wake, params.wake + 2.0, p);
  const t2 = params.wake + (params.cycle - params.wake) * 0.60;
  const t3 = params.wake + (params.cycle - params.wake) * 0.84;
  const sustain = 1 - smoothstepJS(t2, t3, p);
  return {
    frame, time: +time.toFixed(2), ph: +p.toFixed(2),
    act: +(params.flow * attack * sustain).toFixed(3),
    burst: +attack.toFixed(3), sustain: +sustain.toFixed(3),
  };
};
// [调试] 音频状态探针
window.__audioState = () => ({
  on: audioOn,
  ctx: audio ? audio.ctx.state : null,
  booms: booms,
  drone: audio ? audio.droneGain.gain.value.toFixed(4) : null,
});
...
function step(t) {
  ...
  // ---- 声音：阶段音频同步（呼吸周期、爆发轰鸣；低鸣为恒定背景层） ----
  const phNow = time % params.cycle;
  if (audioOn && audio && !paused) {
    audio.lfo.frequency.value = 1 / (7 - 2.5 * burstEnvelope(time)); // 亢奋时呼吸略加快
    if (lastPh < params.wake && phNow >= params.wake) playBoom(1.0); // 爆发瞬间：心跳轰鸣
  }
  lastPh = phNow;
  ...
  if (!paused) {
    ...
    time += dtSmooth;   // 周期按真实秒计；"流动速度"只缩放流场运动（见 uSpeed）
    frame++;
    pulseKeys();
  } else {
...
Promise.all([
  ...
  dreamImg = img;
  setSize();
  initAudio();          // 声音默认开（浏览器政策：首次手势后自动 resume）
  preloadStickers();    // 预载 14 个水钻贴纸 PNG（不阻塞启动）
  const unlock = () => { if (audio.ctx.state === 'suspended') audio.ctx.resume(); };
